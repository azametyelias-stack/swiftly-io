...
| **PHASE-1-VISION-FEATURE-APPLICATIONS.md** | La **vision** de la feature Applications (code interne FEATURE-APPS-PAGE) : pourquoi l'écosystème modulaire, périmètre, personas, métriques, risques, Go / No-Go | Le « pourquoi » de tout ce qui suit — à lire avant les trois autres |
| **ARCHITECTURE-ISOLATION-PERMISSIONS.md** | **Fondation non négociable n°1** : les 3 niveaux de données, le manifeste d'app, la clé d'isolation `(espace, app)`, la couche d'accès unique | Avant d'écrire quoi que ce soit qui touche aux permissions ou aux données d'une app |
| **MIGRATION-MULTI-ESPACES.md** | La refonte de fondation `user_id` → `space_id` : le modèle d'espaces (écosystème, pas SwiftlyTrack), ce qui descend dans l'espace, la stratégie additive en 4 temps | Avant d'écrire une ligne de la migration — et avant la page Applications, qu'elle bloque |
| **MIGRATION-CONTROLES-VALIDATION.md** | Les 4 contrôles **Go / No-Go** de cette migration (orphelines, totaux, un seul espace personnel, isolation) | Annexe du précédent : à chaque passage en test, après le Temps 2 et après le Temps 3 |
| **DECISIONS-MULTI-ESPACES.md** | Les **neuf arbitrages** d'architecture de la migration (A à J) : pourquoi chacun, ce qu'il résout, ce qui casse si on le change | Avant de coder la migration — et le jour où l'on se demande « pourquoi c'est fait comme ça » |
...
**« C'est quoi la feature Applications, et pourquoi l'isolation passe avant tout ? »**
→ `PHASE-1-VISION-FEATURE-APPLICATIONS.md` (la vision, le périmètre, les Go / No-Go)
→ puis `ARCHITECTURE-ISOLATION-PERMISSIONS.md` (fondation n°1 — si l'isolation fuit, on ne lance pas)

**« Pourquoi on migre vers les espaces, et comment on sait que ça n'a rien cassé ? »**
→ `MIGRATION-MULTI-ESPACES.md` (le modèle et les 4 temps)
→ puis `MIGRATION-CONTROLES-VALIDATION.md` (les 4 contrôles Go / No-Go — un seul échec = retour arrière)
→ et `DECISIONS-MULTI-ESPACES.md` (les neuf arbitrages, avec leur raison et leur coût)
...
## ⚠️ Note de vocabulaire — les 4 documents « Applications »

`ARCHITECTURE-ISOLATION-PERMISSIONS.md` et `PHASE-1-VISION-FEATURE-APPLICATIONS.md`
ont été écrits avant `MIGRATION-MULTI-ESPACES.md` et parlent d'`activity_id`. La
décision plus récente prévaut : **en base c'est `space_id`**, et « activité » est un
mot d'interface qui ne désigne que les espaces de type `business`. La clé d'isolation
des apps se lit donc **`(space_id, app_id)`**, et le §5 « une app s'installe par
activité » veut dire : **par espace `business`** — la page Applications n'est jamais
visible dans l'espace personnel.
